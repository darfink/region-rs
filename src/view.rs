use crate::{lock, page, protect, query_range, Protection, Region, Result};
use std::mem;
...
/// OSs page size. It bookmarks the initial and previous (when altered) memory
/// access to all pages within the view. This allows easily restoring memory
/// access for temporary modifications.
///
...
/// std::mem::transmute(ret5.as_ptr()) };     assert_eq!(x(), 5);
///   }).unwrap()
/// }
/// ```
#[derive(Debug, Clone)]
